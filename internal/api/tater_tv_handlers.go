...
type taterTVChannel struct {
	...
	LogoOverlayEnabled *bool            `json:"logoOverlayEnabled,omitempty"`
...
func (s *Server) handleTaterTVLineup(c *fiber.Ctx) error {
	...
	for index := range channels {
		enabled := channels[index].LogoURL != "" && taterTVChannelLogoEnabled(cfg, channels[index])
		channels[index].LogoOverlayEnabled = &enabled
	}
...
func taterTVOrderedSources(cfg *config.Config, baseURL, playerToken string) ([]taterTVSource, error) {
	...
	for _, source := range sources {
		...
		if source.SourceType != "custom" {
			for _, themed := range taterTVThemedSources(source) {
				taterTVApplyAutoChannelLogo(cfg, &themed)
				ordered = append(ordered, themed)
			}
		}
	}
	return ordered, nil
...
func taterTVPersonalizeChannels(channels []taterTVChannel, baseURL, playerToken string) []taterTVChannel {
	...
	for _, channel := range channels {
		...
		next.LogoURL = taterTVChannelLogoURL(baseURL, channel.Number, playerToken, channel.LogoPath)
