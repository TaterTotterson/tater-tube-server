	...
	return r.WithTransaction(ctx, func(tx *Repository) error {
		if _, err := tx.db.ExecContext(ctx, `
			INSERT INTO tater_recommendation_batches
				(id, profile_id, core_id, summary, picks_briefing, boot_summary, generated_at, expires_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?)
		`, batch.ID, batch.ProfileID, batch.CoreID, batch.Summary, batch.PicksBriefing, batch.BootSummary,
...
func (r *Repository) GetActiveTaterRecommendations(ctx context.Context, profileID string, now time.Time) (*TaterRecommendationBatch, []TaterRecommendation, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT batch.id, batch.profile_id, batch.core_id, batch.summary, batch.picks_briefing,
			batch.boot_summary,
	...
	`, profileID, now).Scan(
		&batch.ID, &batch.ProfileID, &batch.CoreID, &batch.Summary, &batch.PicksBriefing,
		&batch.BootSummary,
...
func (r *Repository) ListTaterRecommendationBatches(ctx context.Context, profileID string, limit int) ([]TaterRecommendationBatch, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&item.ID, &item.ProfileID, &item.CoreID, &item.Summary, &item.PicksBriefing,
			&item.BootSummary,
...
func (r *Repository) GetActiveTaterPicksBriefing(
	ctx context.Context,
	batchID, profileID string,
	now time.Time,
) (string, error) {
	var briefing string
	err := r.db.QueryRowContext(ctx, `
		SELECT picks_briefing
		FROM tater_recommendation_batches
		WHERE id = ?
			AND profile_id = ?
			AND expires_at > ?
		LIMIT 1
	`, batchID, profileID, now).Scan(&briefing)
	return briefing, err
}
