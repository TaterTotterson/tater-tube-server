...
func initializeImporter(
	...
	importerService, err := importer.NewService(serviceConfig, metadataService, db, poolManager, configGetter, healthRepo, broadcaster, userRepo)
	if err != nil {
...
func createHTTPServer(apiServer *api.Server, app *fiber.App, streamHandler *api.StreamHandler, port int, configGetter config.ConfigGetter) *http.Server {
	...
	mainHandler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasPrefix(path, "/api/tater/local/stream") {
			localStreamHTTPHandler.ServeHTTP(w, r)
			return
		}
